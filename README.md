...
-   I've noticed that the API doesn't update a comment if it POSTs a blank string (""). So I've mocked it so that a user can "remove" a comment by just entering a single space.
-   I've been wanting to try out React's Context API and it looked like a good fit for this project. However I do realise that the function I'm passing as a value would've been much better served as an action in it's own file, which can be imported and used when necessary. But as it's so closely linked with the expenses page's state, it has to live there for now. If I had a Redux store it would've been very easy to convert this into an action and just pull the necessary values from the global state when a data refresh has to happen.
